...
if len(sys.argv) < 3:
    print("Usage: program <taxa and unicode shape config> <trees>")
    sys.exit(1)
...
with open(cfi_file, "r") as fp:
    for line in fp:
        # common id    color   unicode_shape
        tmp = line.strip().split("\t")
        taxa_labels[tmp[0]] = tmp[1:]
...
for tfile in tree_files:
    ...
    new_tfile = os.path.join(os.path.dirname(tfile), "l_{}.clusters.nexus".format(os.path.basename(tfile).rsplit(".",1)[0]))
        ...
        for node in tr.traverse("postorder"):
            if node.name:
                if node.name[0] != "'" or node.name[0] != node.name[-1]:
                    ...
                    apostr_nn = node.name
                common = False
                for taxa_name in taxa_labels.keys():
                    if node.name.find(taxa_name) > -1:
                        print("\t{0}[&!color={1},taxalabel={2}]".format(apostr_nn, taxa_labels[taxa_name][0],taxa_labels[taxa_name][1]), file=of)
                        common = True
                        break
                if not common:
